...
from backend.models import CustomInstruction, ChatSession, ChatMessage, MessageAttachment
...
def test_put_custom_instructions_missing_fields_422():
    ...
    res = client.put("/api/v1/custom-instructions", json=payload)
...
def test_put_idempotency_identical_payload():
    ...
    res1 = client.put("/api/v1/custom-instructions", json=body)
    assert res1.status_code == 200
    updated_at_1 = res1.json()["updated_at"]

    # 동일 요청 2회 연속 수행
    res2 = client.put("/api/v1/custom-instructions", json=body)
    assert res2.status_code == 200
